...
class IMDB_2(TabularDataset):
    ...
    TEXT_FIELD = Field(lower=True, batch_first=True, tokenize=Clean_Str, include_lengths=False)
    ...
    def iters(cls, path, vectors_name, vectors_cache, batch_size=64, shuffle=True, device=0, vectors=None,
        ...
        return BucketIterator.splits((train, val, test), batch_size=batch_size, repeat=False, shuffle=shuffle,
                                     sort_within_batch=True, sort_key=lambda x:len(x.text), device=device)
...
if __name__ == "__main__":
    ...
    model = DPCNN(config).to(args.gpu)
